...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for parent in parents:
        ...
        parent_paras = [p.strip() for p in parent.text.split("\n\n") if p.strip()]
        current = ""
        for para in parent_paras:
            if len(current) + len(para) > child_size and current:
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    return chunks if chunks else [Chunk(text=text, metadata={**metadata, "strategy": "structure"})]
